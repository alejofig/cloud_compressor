...
sed -E "s#export DATABASE_URL=postgresql://example:example@[0-9\.]*:5432/example#DATABASE_URL: postgresql://example:example@${database_id}:5432/example#" process_files.sh > process_files-temp.sh && mv  process_files-temp.sh process_files.sh
