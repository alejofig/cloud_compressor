...
sed -E "s#DATABASE_URL: postgresql:/postgres:example@.*:5432/example#DATABASE_URL: postgresql:/postgres:example@${database_id}:5432/example#" docker-compose.yml > docker-compose-temp.yml && mv docker-compose-temp.yml docker-compose.yml
...
sed -E "s#DATABASE_URL: postgresql:/postgres:example@.*:5432/example#DATABASE_URL: postgresql:/postgres:example@${database_id}:5432/example#" docker-compose-worker.yml > docker-compose-worker-temp.yml && mv docker-compose-worker-temp.yml docker-compose-worker.yml
...
sed -E "s#export DATABASE_URL=postgresql:/postgres:example@.*:5432/example#export DATABASE_URL=postgresql:/postgres:example@${database_id}:5432/example#" process_files.sh > process_files-temp.sh && mv  process_files-temp.sh process_files.sh
